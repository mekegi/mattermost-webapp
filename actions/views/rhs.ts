...
export function performSearch(terms: string, isMentionSearch?: boolean) {
    return (dispatch: DispatchFunc, getState: GetStateFunc) => {
        ...
        const messagesPromise = dispatch(searchPostsWithParams(
            isMentionSearch ? '' : teamId,
            {terms, is_or_search: Boolean(isMentionSearch),
                include_reactions: Boolean(isMentionSearch),
        ...
        const filesPromise = dispatch(searchFilesWithParams(teamId, {
            ...
            terms: termsWithExtensionsFilters, is_or_search: Boolean(isMentionSearch), include_deleted_channels: viewArchivedChannels, time_zone_offset: timezoneOffset, page: 0, per_page: 20}));
